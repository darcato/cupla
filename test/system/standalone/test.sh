...
function compile {
    ...
    if [ $ret -ne 0 ] ; then
        echo "Standalone header test failed for accelerator '$acc_name' with compiler flags '$compiler_flags'" >&2
    fi
    return $ret
}
...
compile 1 "$1" CpuOmp2Blocks "-fopenmp -DCUPLA_STREAM_ASYNC_ENABLED=1 $boost_include"
compile 0 "$1" CpuOmp2Threads "-fopenmp $boost_include"
compile 0 "$1" CpuOmp2Threads "-fopenmp -DCUPLA_STREAM_ASYNC_ENABLED=1 $boost_include"
compile 1 "$1" CpuSerial "$boost_include"
compile 1 "$1" CpuSerial "-lpthread -DCUPLA_STREAM_ASYNC_ENABLED=1 $boost_include"
compile 0 "$1" CpuThreads "-lpthread $boost_include"
compile 0 "$1" CpuThreads "-lpthread -DCUPLA_STREAM_ASYNC_ENABLED=1 $boost_include"

nvcc_found=$(which nvcc >/dev/null && { echo 0; } || { echo 1; })
if [ $nvcc_found -eq 0 ] ; then
    compile 0 "nvcc" GpuCudaRt "$boost_include"
    compile 0 "nvcc" GpuCudaRt "-DCUPLA_STREAM_ASYNC_ENABLED=1 $boost_include"
else 
...
if [ $hipcc_found -eq 0 ] ; then
    ...
    compile 1 "hipcc" GpuHipRt "$hip_include $boostIncude"
    compile 1 "hipcc" GpuHipRt "-lpthread -DCUPLA_STREAM_ASYNC_ENABLED=1 $hip_include $boost_include"
else 
